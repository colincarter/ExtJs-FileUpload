    ...
    this.progressTask = {
      run: function() {
        Ext.Ajax.request({
          url: config.progress,
          method: 'GET',
          params: {ID: uploadIdentifier},
          success: function(r) {
            if (r.responseText == 'null') {
              return;
            }
            var response = Ext.decode(r.responseText);
            var total = parseInt(response.bytes_total);
            var uploaded = parseInt(response.bytes_uploaded);
            var progress = uploaded/total;
            progressBar.updateProgress(progress, Math.round(100*progress)+'% completed...', true);
          }
        });
      },
      interval: 500, // 500ms
